...
egg_tray_icon_send_manager_message(EggTrayIcon * icon,
								   long message,
								   Window window, long data1, long data2, long data3)
{
	...
	if (icon->manager_window != None) {
		...
		gdkwin =
			gdk_window_lookup_for_display(gtk_widget_get_display
										  (GTK_WIDGET(icon)), icon->manager_window);
#else
		gdkwin = gdk_window_lookup(icon->manager_window);
#endif
						   ...
						   gint timeout, const gchar * message, gint len)
{
	...
	egg_tray_icon_send_manager_message(icon, SYSTEM_TRAY_BEGIN_MESSAGE, (Window)
...
egg_tray_icon_cancel_message(EggTrayIcon * icon, guint id)
{
	...
	egg_tray_icon_send_manager_message(icon, SYSTEM_TRAY_CANCEL_MESSAGE, (Window)
									   gtk_plug_get_id(GTK_PLUG(icon)), id, 0, 0);
}
