...
void
   Updater::downloadIcon(QString & url, QString & filename)
{
   HttpDownloader *
      dl = new HttpDownloader(url, filename, this);
   ...
   Updater::downloadTeamIcon(qint32 id)
{
   if (id == 0)
      return;
   QString
      url = "http://mononoke-bt.org/pic/teams/icons/" + QString::number(id) + ".jpg";
   QString
      filename = QDir::homePath() + "/.config/mononoke-notify-2/data/team_"
   ...
   Updater::downloadAnimeIcon(qint32 id)
{
   if (id == 0)
      return;
   QString
      url = "http://mononoke-bt.org/pic/categories/" + QString::number(id) + ".jpg";
   QString filename = QDir::homePath()
   ...
   Updater::downloadGenreIcon(qint32 id)
{
   if (id == 0)
      return;
   //return; /*\ WARNING !!! \*/
   QString url = "http://mononoke-bt.org/pic/genres/" + QString::number(id) + ".jpg";
   QString filename = QDir::homePath()
...
{
   QFile
      file(QDir::homePath() + "/.config/mononoke-notify-2/themes/"
...
void
   Updater::checkThemeFiles(QDomElement & elem, QString & base)
{
   QString local;
   QString url;
   HttpDownloader *
